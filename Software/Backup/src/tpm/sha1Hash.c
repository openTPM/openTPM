...
void sha1_update_t( hashContext *ctx, uint8_t *input, uint32_t length )
{
    ...
    if( left && length >= fill )
    {
        memcpy( (void *) (ctx->buffer + left),
                (void *) input, fill );
        sha1_process_t( ctx, &ctx->buffer[0]);
        length -= fill;
        input  += fill;
        left = 0;
    }
...
void sha1_finish_t( hashContext *ctx, uint8_t *digest)
{
    ...
    sha1_update_t( ctx, sha1_padding, padn );
    sha1_update_t( ctx, msglen, 8 );
...
void sha1_csum_t( uint8_t *buf, uint32_t buflen, uint8_t *digest )
{
	...
	static hashContext *ctx;// = (hashContext*) workBuf3;
    ctx = (hashContext*) workBuf3;
	sha1_starts_t( ctx );
    sha1_update_t( ctx, buf, buflen );
    sha1_finish_t( ctx, digest );
}
                ...
                uint8_t* digest )
{
    ...
    sha1_starts_t( ctx );
    sha1_update_t( ctx, k_ipad, 64 );
    sha1_update_t( ctx, buf, buflen );
    sha1_finish_t( ctx, tmpbuf );

    sha1_starts_t( ctx );
    sha1_update_t( ctx, k_opad, 64 );
    sha1_update_t( ctx, tmpbuf, 20 );
    sha1_finish_t( ctx, digest );
...
uint8_t sha1_self_test( )
{
	...
	sha1_csum_t((uint8_t *) sha1_test_str[i], strlen( sha1_test_str[i] ), (uint8_t *)sha1sum);
...
// 
//         if( i < 2 )
//             sha1_update_t( &ctx, (uint8_t *) sha1_test_str[i],
//                          strlen( sha1_test_str[i] ) );
//         else
//         {
//             memset( buf, 'a', 1000 );
//             for( j = 0; j < 1000; j++ )
//                 sha1_update_t( &ctx, (uint8_t *) buf, 1000 );
//         }
// 
//         sha1_finish_t( &ctx, sha1sum );
// 
//         if( memcmp( sha1sum, sha1_test_sum[i], 20 ) != 0 )
//         {
// 			asm("nop");
...
//     	for(i=0; i < 5; i++)
//     		buf[i] = i;
//     	sha1_csum( buf, 5, sha1sum);
//         //printf( "my SHA-1 test :\n" );
//     	for(i=0; i < 20; i++)
//     		//printf("%02x ", sha1sum[i]);
//         //printf( "\n" );
//     
//     	// load up the HMAC key
//     	for(i=0; i<20; i++)
//     		key[i] = 0xA5;
//     
...
//     	                uchar digest[20] )
//     
//     	sha1_hmac( key, sizeof(key), buf, 5, sha1sum );
//         //printf( "my HMAC test\n" );
//     	for(i=0; i < 20; i++)
//     		//printf("%02x ", sha1sum[i]);
//         //printf( "\n" );
